# Hi, I'm Kaveri K K 👋

🎓 B.E. Artificial Intelligence & Machine Learning student at B.M.S. College of Engineering, Bengaluru (CGPA: 8.6/10)
🤖 Building projects across ML, systems, and full-stack development
📫 Reach me: sinchankaveri30@gmail.com · [LinkedIn](https://linkedin.com/in/kaverikk30)

## 🚀 Featured Projects

| Project | Description |
|---|---|
| 🚑 [RescueRoute](https://github.com/SinchanKaveri/rescueroute) | Real-time ambulance dispatch system using Dijkstra's algorithm — 32% faster response time |
| 🌾 [Crop Disease & Water Stress Detection](https://github.com/SinchanKaveri/crop-disease-detection) | Multi-modal AI pipeline (drone imagery + IoT) — 99.67% detection accuracy |
| ☁️ [CloudFlow](https://github.com/SinchanKaveri/cloudflow) | CPU scheduling simulator comparing FCFS/SJF/Round Robin — 43% faster avg. waiting time |
| 💬 [AI Chatbots](https://github.com/SinchanKaveri/academic-chatbot) | NLP academic assistant + Claude-powered mental health chatbot (MindEase) |

## 🛠️ Tech Stack

**Languages:** Python, Java, C, C++, HTML, CSS
**ML/AI:** Scikit-learn, TensorFlow, NLTK, spaCy
**Frameworks:** Flask, React.js
**Tools:** Git, GitHub, VS Code, IntelliJ, Jupyter Notebook

## 📚 Currently Learning
Graph theory, systems design, and applied AI engineering for large-scale platforms.

## 🏆 Certifications
- NPTEL – Engineering Ethics
- Infosys Springboard – Java 11 Essentials

---
⭐️ Feel free to explore my repos and connect!
